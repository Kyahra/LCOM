...
/**
 * @brief handler to print the scancodes
 *
 *@param if 0 call c_handler if 1 call asm_handler
 *
 * @return Return 0 upon success and non-zero otherwise or when the cycle must end
 */
int handler(unsigned short ass);

unsigned long asm_handler(unsigned long code);
